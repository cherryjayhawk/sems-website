...
function Area() {
	...
	const handleSubmit = async (e) => {
		...
		try{
			setPost(true)
			const response = await fetch(`https://sems-webservice-ten.vercel.app/api/areas`, {
				method: 'POST',
				body: JSON.stringify({
						...
						<form className='w-1/2 m-4' onSubmit={handleSubmit}>
							<div className="flex flex-col w-full">
								...
								<div className="flex my-4">
									...
									<input required type="text" name='area' id="area" className="border-2 px-4 py-1 w-96 text-[--contrast-color] bg-[--button-primary] border-[--button-primary]" onChange={e => setArea(e.target.value)}/>
								</div>
								<div className="flex my-4">
									<label htmlFor="site" className="w-32 font-semibold">Lokasi</label>
									<select required name='site' defaultValue={sites[0]?.id} className="border-2 px-4 py-1 w-96 text-[--contrast-color] bg-[--button-primary] border-[--button-primary]" onChange={e => setSite(e.target.value)}>
                                    {
										sites?.map((site) => {
											return (
